        ...
        try {
            ...
            try {
                ...
                if (tmpEnc != null)
                    ...
                    Logger.d("snacktory", "no charset found in first stage");
                    // detect with the help of xml beginning ala encoding="charset"
                    tmpEnc = detectCharset("encoding=", output, in, encoding);
                    if (tmpEnc != null)
                        encoding = tmpEnc;
                    else
                        Logger.d("snacktory", "no charset found in second stage");
                }

                if (!Charset.isSupported(encoding))
                    throw new UnsupportedEncodingException(encoding);
            } catch (UnsupportedEncodingException e) {
                Logger.w("snacktory", "Using default encoding:" + UTF8
            ...
            while (true) {
                if (bytesRead >= maxBytes) {
                    Logger.w("snacktory", "Maxbyte of " + maxBytes + " exceeded! Maybe html is now broken but try it nevertheless. Url: " + url);
                    break;
                }
            ...
            Logger.i("snacktory", e.toString() + " url:" + url);
        } catch (IOException e) {
            Logger.w("snacktory", e.toString() + " url:" + url);
        } finally {
            if (in != null) {
                try {
                    in.close();
        ...
        if (encIndex > 0) {
            ...
            if (lastEncIndex > encIndex + clength && lastEncIndex < encIndex + clength + 40) {
                ...
                try {
                    ...
                    Logger.w("snacktory", "Couldn't reset stream to re-read with new encoding " + tmpEnc + " "
                            + ex.toString());
                }
            }
        }
